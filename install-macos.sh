...
#
# ╔═══════════════════════════════════════════════════════════════════╗
# ║  LogosAI Installer — macOS                                       ║
# ║  Multi-Agent AI Platform — Full Stack Setup                       ║
# ║                                                                   ║
# ║  Usage:                                                           ║
# ║    curl -fsSL https://raw.githubusercontent.com/                  ║
# ║      maior/logosai-framework/main/install-macos.sh | bash         ║
# ║                                                                   ║
# ║  Prerequisites: Homebrew, Python 3.11+, Node 18+, PostgreSQL 14+ ║
...
if [[ "$(uname)" != "Darwin" ]]; then
    ...
    echo "  For Ubuntu/Debian, use: install-ubuntu.sh"
    echo ""
    exit 1
fi
...
SCRIPTS_ONLY=false
for arg in "$@"; do
    [ "$arg" = "--scripts-only" ] && SCRIPTS_ONLY=true
done
...
if [ "$SCRIPTS_ONLY" = true ]; then
    ...
    current_step=4
    step "Regenerating management scripts"
    # Fall through to Step 5 section at the bottom
else
...
ASCII
echo -e "${NC}"
echo -e "  ${BOLD}${W}Multi-Agent AI Platform${NC}  ${DIM}v1.0 — macOS${NC}"
...
echo -e "    ${DIM}1.${NC} Check & install prerequisites via Homebrew"
echo -e "    ${DIM}2.${NC} Clone 4 open-source repositories"
...
if ! command -v brew &>/dev/null; then
    ...
    dim "    Install: ${W}/bin/bash -c \"\$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)\"${NC}"
    MISSING=1
else
    ok "Homebrew ${W}$(brew --version | head -1 | cut -d' ' -f2)${NC}"
fi
...
# Extra packages often missing
pip install asyncpg aiohttp psycopg2-binary -q 2>&1 | grep -iE "error|ERROR" | head -3 || true
...
(cd logosai-web && npm install --legacy-peer-deps --loglevel=error 2>&1 | grep -iE "error|ERR" | head -3)
...
echo ""
warn "macOS: Grant Accessibility permission to Terminal in"
...
# set_env_key — uses Python for reliable .env updates
set_env_key() {
...
if [ "$PG_OK" -eq 2 ]; then
    ...
    for attempt in 1 2 3; do
        ...
        # macOS Homebrew PostgreSQL typically uses trust auth for local, not peer
...
# Build logos_web (AFTER .env.local is configured)
echo ""
info "Building logos_web for production..."
dim "  (this may take 1-2 minutes)"
(cd logosai-web && npx next build 2>&1 | tail -3) || true
ok "${W}logos_web${NC} production build"

fi  # end of SCRIPTS_ONLY check
...
G='\033[0;32m'; R='\033[0;31m'; Y='\033[1;33m'; B='\033[0;34m'; P='\033[0;35m'; C='\033[0;36m'
...
#!/usr/bin/env bash
#
# LogosAI — Live Service Monitor (macOS)
#
DIR="$(cd "$(dirname "$0")" && pwd)"
...
get_uptime() {
    ...
    if [ -f "$pidfile" ]; then
        ...
        if [ -n "$pid" ] && kill -0 "$pid" 2>/dev/null; then
            local start_time=$(stat -f %m "$pidfile" 2>/dev/null)
...
draw_services() {
    ...
    for svc in "${SERVICE_ORDER[@]}"; do
        ...
        if [ "$status" = "running" ]; then
            ...
            [ "$http" = "000" ] && { status_icon="◐"; status_color="$Y"; }
        else
            status_icon="■"; status_color="$R"; uptime="-"; pid="-"
        fi
...
draw_single_log() {
    ...
    if [ -f "$DIR/logs/${svc}.log" ]; then
        tail -${log_lines} "$DIR/logs/${svc}.log" 2>/dev/null | while read -r line; do echo "  $line"; done
    else
...
while true; do
    ...
    draw_header
    case "$VIEW" in
        overview) REFRESH=5; draw_services; draw_logs ;;
        acp|api|web) REFRESH=3; draw_services; draw_single_log "$VIEW" ;;
    esac
    if read -t $REFRESH -n 1 key 2>/dev/null; then
        case "$key" in
            q|Q) tput cnorm 2>/dev/null; echo ""; exit 0 ;;
            r|R) continue ;;
            0) VIEW="overview" ;; 1) VIEW="acp" ;; 2) VIEW="api" ;; 3) VIEW="web" ;;
        esac
    fi
done
MONITOREOF
...
if [ -d "$DIR/logosai-web/node_modules" ]; then
    ...
    (cd "$DIR/logosai-web" && npm install --legacy-peer-deps --loglevel=error 2>&1 | grep -iE "error|ERR" | head -3)
...
curl -fsSL https://raw.githubusercontent.com/maior/logosai-framework/main/install-macos.sh \
    | LOGOSAI_DIR="$DIR" bash -s -- --scripts-only 2>/dev/null \
    || echo -e "  ${DIM}Scripts regeneration skipped (run install-macos.sh to update)${NC}"
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
if [ "$SCRIPTS_ONLY" = true ]; then
    ok "Management scripts regenerated"
    exit 0
fi
...
echo -e "  ${DIM}Completed in ${W}${MINUTES}m ${SECS}s${NC}"
echo ""
echo -e "  ${DIM}┌──────────────────────────────────────────────────────┐${NC}"
