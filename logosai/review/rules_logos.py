"""Logos 규칙셋 — 이 코드베이스의 헌법이자 `rules.py` 메커니즘의 **참조 인스턴스**.

여기 13개는 보편 규칙이 아니다. **이 시스템이 실제로 겪은 사고**에서 나왔다.
다른 조직이 이 SDK 를 쓴다면 여기를 그대로 물려받는 대신, 자기 사고로 자기
`RuleSet` 을 만들어 주입하는 것이 맞다 — 남의 사고에서 나온 규칙은 자기
코드베이스에서 소음이 되고, 소음은 무시를 낳는다.

그럼에도 이 파일을 공개하는 이유: 참조 구현이 없으면 `incident` 를 무엇으로
채워야 하는지, 규칙이 어느 정도로 좁아야 하는지 아무도 모른다. 그리고 근거를
지운 규칙 목록은 취향 목록이므로, 감사받을 수 없는 헌법이 된다 (사상 ⑧).

어느 규칙이 실제로 값진지는 측정이 알려준다
──────────────────────────────────────
정밀도 감사(G2, 2026-08-22) 실측 — 같은 6개 파일:
...
프롬프트를 다듬는 작업은 61%→51% 로 **악화**시켰고, 솎기는 진짜 발견 88% 를
지키며 80% 로 올렸다. 규칙은 사고로 심사받고, 값을 못 내는 규칙은 솎아낸다.
솎기는 `RuleSet.subset()` 또는 `build_review_prompt(rule_ids=...)` 로 한다.
...
DEFAULT_RULES: RuleSet = RuleSet.of(
    Rule(
        id="R-001",
...
#: 하위호환 — 기존 코드가 `from .rules import RULES` 를 썼다.
