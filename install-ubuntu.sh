...
#
# ╔═══════════════════════════════════════════════════════════════════╗
# ║  LogosAI Installer — Ubuntu / Debian                              ║
# ║  Multi-Agent AI Platform — Full Stack Setup                       ║
# ║                                                                   ║
# ║  Usage:                                                           ║
# ║    curl -fsSL https://raw.githubusercontent.com/                  ║
# ║      maior/logosai-framework/main/install-ubuntu.sh | bash        ║
# ║                                                                   ║
...
if [ -f /etc/os-release ]; then
    ...
    if [[ "$DISTRO" != "ubuntu" && "$DISTRO" != "debian" && "$DISTRO" != "linuxmint" && "$DISTRO" != "pop" ]]; then
        ...
        echo "  Detected: $DISTRO. Proceeding anyway..."
        echo ""
    fi
else
    DISTRO="linux"
...
ASCII
echo -e "${NC}"
echo -e "  ${BOLD}${W}Multi-Agent AI Platform${NC}  ${DIM}v1.0 — Ubuntu/Debian${NC}"
...
echo -e "    ${DIM}1.${NC} Check & install prerequisites via apt"
...
# ── Desktop Agent dependencies (Ubuntu) ──
echo ""
...
sudo apt-get install -y xdotool xclip scrot python3-tk python3-dev 2>&1 | tail -3 || {
    warn "Some desktop tools could not be installed (non-critical)"
}
...
if [ "$PG_OK" -eq 2 ]; then
    ...
    # ── Connection test with retry + peer auth handling ──
    for attempt in 1 2 3; do
        ...
        if [ "$attempt" -eq 1 ]; then
            # Check if it's a peer auth issue (common on Ubuntu)
            if sudo -u "$DB_USER" psql -d postgres -c "SELECT 1" >/dev/null 2>&1; then
                ...
                # Enable md5 auth in pg_hba.conf
...
# Open browser on desktop (not SSH/headless)
...
#!/usr/bin/env bash
#
# LogosAI — Live Service Monitor (Ubuntu/Debian)
#
DIR="$(cd "$(dirname "$0")" && pwd)"
...
get_uptime() {
    ...
    if [ -f "$pidfile" ]; then
        ...
        if [ -n "$pid" ] && kill -0 "$pid" 2>/dev/null; then
            local start_time=$(stat -c %Y "$pidfile" 2>/dev/null)
...
curl -fsSL https://raw.githubusercontent.com/maior/logosai-framework/main/install-ubuntu.sh \
    | LOGOSAI_DIR="$DIR" bash -s -- --scripts-only 2>/dev/null \
    || echo -e "  ${DIM}Scripts regeneration skipped (run install-ubuntu.sh to update)${NC}"
